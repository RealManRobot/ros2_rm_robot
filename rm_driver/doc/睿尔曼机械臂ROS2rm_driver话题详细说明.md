...
| 功能描述 | 清除关节错误代码 |
| :---: | :----: |
| 参数说明 | Jointerrclear.msg uint8 joint_num：对应关节序号，从基座到机械臂手爪端，序号依次为1～6。bool block：是否为阻塞模式，bool类型，true:阻塞，false:非阻塞 |
| 命令示例 | ros2 topic pub /rm_driver/set_joint_err_clear_cmd rm_ros_interfaces/msg/Jointerrclear "joint_num: 1
block: true" |
| 返回值 | true-设置成功，false-设置失败 |
| 返回话题 | ros2 topic echo /rm_driver/set_joint_err_clear_result |
...
block: true 
dof: 7"
返 值	成功返回：true；失败返回：false，driver终端返回错误码。
...
expand: 0.0
dof: 6"
返 值	成功：无返回值；失败返回：driver终端返回错误码。
...
返 值	成功返回：版本信息；失败返回：driver终端返回错误码。
...
返 值	成功返回：升降机构当前状态；失败返回：driver终端返回错误码。
...
follow: false
dof: 6
返 值	成功无返回；失败返回：false，driver终端返回错误码。
...
订阅命令	ros2 topic echo /rm_driver/udp_six_force
一维力
功能描述	一维力
参数说明	Sixforce.msg
float32 force_fx：沿x轴方向受力大小。
...
float32 force_my：沿y轴方向转动受力大小。
float32 force_mz：沿z轴方向转动受力大小。
订阅命令	ros2 topic echo /rm_driver/udp_one_force
机械臂错误
功能描述	机械臂错误
...
参数说明	std_msgs::msg::UInt16
uint16 data：系统报错信息。
订阅命令	ros2 topic echo /rm_driver/udp_sys_err
关节错误
功能描述	关节错误
参数说明	Jointerrorcode.msg
uint16[] joint_error：每个关节报错信息。
Uint8 dof：机械臂自由度信息。
订阅命令	ros2 topic echo /rm_driver/udp_joint_error_code
机械臂弧度数据
功能描述	机械臂弧度数据
...
订阅命令	ros2 topic echo /joint_states
位姿信息
功能描述	位姿信息
...
Quaternion orientation：机械臂当前姿态信息。
	...
	float64 z 0
	float64 w 1
订阅命令	ros2 topic echo /rm_driver/udp_arm_position
当前六维力传感器系统外受力数据
...
订阅命令	ros2 topic echo /rm_driver/udp_six_zero_force
当前一维力传感器系统外受力数据
...
float32 force_my：当前传感器沿y轴方向转动受外力大小。
float32 force_mz：当前传感器沿z轴方向转动受外力大小。
订阅命令	ros2 topic echo /rm_driver/udp_one_zero_force
系统外受力数据参考坐标系
功能描述	系统外受力数据参考坐标系
参数说明	std_msgs::msg::UInt16
uint16 data：系统外受力数据的坐标系，0 为传感器坐标系 1 为当前工作坐标系 2 为当前工具坐标系。该数据会影响一维力和六维力传感器系统外受力数据的参考坐标系
订阅命令	ros2 topic echo /rm_driver/udp_arm_coordinate
